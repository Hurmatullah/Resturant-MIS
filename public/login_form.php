			...
			if($user_role==1) {
			 ...
			 //	finance dept
				...
				//kitchen
					...
					$_SESSION['user'] = $username;
					$_SESSION['timeout'] = $currentTime + $expired;
					header("location: manage_kitchen.php");					
				// qail dept	
				}else if($user_role==4) {
					$_SESSION['user'] = $username;
					$_SESSION['timeout'] = $currentTime + $expired;
					header("location: manage_qail.php");					
				// juice dept
				}else if($user_role==5) {
					$_SESSION['user'] = $username;
					$_SESSION['timeout'] = $currentTime + $expired;
					header("location: manage_juice.php");					
				}
     ...
     <form class="login-form" method="post">
        <div class="row">
          <div class="input-field col s12 center">
            ...
            <p class="center login-form-text">Karim Continintal</p>
          </div>
        </div>
        ...
        <div class="row">
          <div class="input-field col s12">
            <button class="btn waves-effect waves-light col s12" type="submit" name="btnLogin">ورود به سیستم</button>
          </div>
        </div>
