...
<?php
	...
	$s = mysqli_query($conn,"select * from order_list_view where res_status = 0 AND order_id='$ID' and category_id = 4  group by order_id order by order_id DESC"); // شرط
 ...
 ?>
    ...
    <?php while($data = mysqli_fetch_array($s)){ ?>
		...
		<tr style="padding:0;">
			...
			$menu_list = mysqli_query($conn,"select * from order_list_view where order_id = '$data[0]' and category_id = 4 ");// شرط
