...
<meta http-equiv="refresh" content="5"/>
	...
	<?php 
		...
		if(empty($keyword)){
			$sql_query = "SELECT order_id, waiter_name, number_of_people, date_time, phone, res_status
				FROM order_list_view where category_id = 1  and res_status = 0 group by order_id 
		...
		// get all data from pemesanan table fire the query
		if(empty($keyword)){
			$sql_query = "SELECT order_id, waiter_name, number_of_people, date_time, phone, res_status
				FROM order_list_view where category_id = 1 and res_status = 0 group by order_id  
				ORDER BY order_id DESC
          	...
          	<div class="container">
            	<div class="row">
              		<div class="col s12 m12 l12">
               			<h5 class="breadcrumbs-title">مدیریت آشپزخانه</h5>
		                <ol class="breadcrumb">
	...
	<!-- ---------------------------------------  SEARCH ON TABLE WHILE DATA ON TABLE EXIST------------------------------------- -->	
							                    ...
							                    <label for="first_name">جستجو به اساس میر نمبر</label>
							                </div></div>
				                		</td>
				                		<td
										div class="input-field col s12">
											...
											<?php 
												while ($stmt_paging->fetch()){ ?>
													...
													<tr>
														...
														<td><div align="center" style="font-size:1.5em;background-color:dodgerblue;border-radius:10px;color:white; padding:2px; width:50px; height:50px;"><?php echo $data['phone'];?></div></td>
														...
														<td><div align="right">
															<a href="detail-reservation.php?id=<?php echo $data['order_id'];?>">
															<i class="mdi-action-pageview"></i>
															</a>
															<a href="delete-reservation.php?id=<?php echo $data['order_id'];?>">
