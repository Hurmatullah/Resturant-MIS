...
<!-- Cancel Report Connection to Database -->
...
// <------------------ Selecting of data from database using specific condition --------------------------->
		...
		$cancelQuery = "SELECT * FROM order_list_manage WHERE date_time between '$from_date' and '$to_date' and res_status = 3" ;

		$cancel = mysqli_query($conn , $cancelQuery);

	    $dis_per = "SELECT sum(price*qty)/100*discount_percentage as dis FROM order_list_manage WHERE date_time between '$from_date' and '$to_date' and res_status = 3 group by order_id" ;

		$result1 = mysqli_query($conn , $dis_per);		
		
		 $dis_amnt = "SELECT sum(discount_amount) as amount FROM order_list_manage WHERE date_time between '$from_date' and '$to_date' and res_status = 3 group by order_id";
